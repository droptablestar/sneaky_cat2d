...
**Player Controller** (`scripts/player_controller.gd`)
...
**Visual Components**
- `scripts/player_visuals.gd` and `scripts/enemy_visuals.gd` handle visual representation
...
├── FollowCamera (Camera3D) - scripts/follow_camera.gd
├── Level (Node3D)
│   ├── Floor (StaticBody3D)
...
- `scripts/` - All GDScript files (single responsibility per script)
