...
func test_player_subviewport_pipeline_configured() -> void:
	...
	assert_true(
		...
		"Player sprite viewport must have non-zero size"
	)
	...
	assert_eq(
		NodePath("Visuals/SpriteViewport"),
		texture.viewport_path,
		"Viewport path should point to SpriteViewport"
	)
	var resolved_viewport := player.get_node(texture.viewport_path) as SubViewport
	assert_not_null(
		resolved_viewport, "ViewportTexture path should resolve to an actual SubViewport"
	)
...
func test_enemy_subviewport_pipeline_configured() -> void:
	...
	var viewport: SubViewport = visuals.get_node("SpriteViewport")
	assert_true(
		viewport.size.x > 0 and viewport.size.y > 0, "Enemy viewport must have non-zero size"
	)
	var billboard: Sprite3D = visuals.get_node("BillboardSprite")
	var texture := billboard.texture as ViewportTexture
	assert_not_null(texture, "Enemy billboard texture must be a ViewportTexture")
	assert_eq(
		NodePath("Visuals/SpriteViewport"), texture.viewport_path, "Enemy viewport path mismatch"
	)
	...
	assert_true(
		sprite.sprite_frames.has_animation("walk"), "Dog sprite frames should define walk animation"
	)
