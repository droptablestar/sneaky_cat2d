...
func test_gravity_applies_when_airborne() -> void:
	...
	assert_true(is_equal_approx(player.velocity.y, expected), "Gravity should decrease Y velocity while airborne")
...
func test_jump_does_not_fire_in_air() -> void:
	...
	assert_true(player.velocity.y < player.jump_velocity, "Airborne attempt should still lose velocity due to gravity")
...
func test_jump_applies_when_on_floor() -> void:
	...
	assert_true(player.global_position.y > start_height, "Player should move upward right after jumping")
...
func test_horizontal_axis_controls_velocity() -> void:
	...
	assert_true(is_equal_approx(player.velocity.x, player.move_speed), "Pressing ui_right should move player right")
	press_action("ui_left")
	player._physics_process(1.0 / 60.0)
	release_action("ui_left")
	assert_true(is_equal_approx(player.velocity.x, -player.move_speed), "Pressing ui_left should move player left")
...
func test_z_plane_constraint_holds_position() -> void:
	...
	player._physics_process(1.0 / 60.0)
	assert_true(is_equal_approx(player.global_position.z, plane_z), "Player should be constrained to initial Z plane")
